...
TypeDisaster_county_Formal=read_excel("~/Library/Mobile Documents/com~apple~CloudDocs/Project/City Resilience/Src/Disasters_Classified/TypeDisaster_county_Formal_HW35_37.xlsx")
...
# Hospitalizations
Case_all=readRDS("~/Library/Mobile Documents/com~apple~CloudDocs/Project/Mental/Src/Data_Mental.rds")

# ======================= Manipulation ===========================

colnames(Case_all)[colnames(Case_all) == "County.x"] <- "County"

colnames(Case_all)[colnames(Case_all) == "date"] <- "datetime"
#Case_all$datetime <- force_tz(Case_all$datetime,tzone='UTC')

Case_all$datetime <- ymd(Case_all$datetime) %>% 
  force_tz(tzone = "UTC")
...
Event_type="Flood"        # Storm, Flood, Cyclone, WinterStorm, Sand, Drought # Heatwave(DryHeatwave, HumidHeatwave)
Event_level="ALL"           # all level - ALL; Billion - B; Million - M
StudyRegion="Disaster"     # "Spillover","Disaster"
ResilienceType="Comprehensive"        # HSR - Healthcare system resilience; Comprehensive - Regional climate resilience
...
if(ResilienceType=="Comprehensive"){
...
# Save the disaster region
Event_info_Disaster=Event_info
...
if(StudyRegion=="Disaster"){
  Event_info=Event_info_Disaster
}
...
for(k in 1:nrow(Event_info)){
  ...
  if(length(matching_row)>0){
    ...
    pre=14 # Observation window control: 
    # Enduring: CaseDay
    # Warning, Post, After = 14 
    warn=14
    post=14
    after=14
    LagA=14
    ...
    LagA_row=(Start_row+No_subgroup*(CaseDay+post+after)):(Start_row+No_subgroup*(CaseDay+post+after+LagA)-1)
    LagB_row=(Start_row+No_subgroup*(CaseDay+post+after+LagA)):(Start_row+No_subgroup*(CaseDay+post+after+LagA+LagB)-1)
    LagC_row=(Start_row+No_subgroup*(CaseDay+post+after+LagA+LagB)):(Start_row+No_subgroup*(CaseDay+post+after+LagA+LagB+LagC)-1)
    ...
    df_Prepost_LagA=df_Case[LagA_row,]
    df_Prepost_LagA$case=val_LagA
    
    df_Prepost_LagB=df_Case[LagB_row,]
    df_Prepost_LagB$case=val_LagB
    
    df_Prepost_LagC=df_Case[LagC_row,]
    df_Prepost_LagC$case=val_LagC
    
    
    
    df_Prepost_temp=rbind(df_Prepost_Pre,df_Prepost_Warn,df_Prepost_Case,df_Prepost_Post,df_Prepost_After,df_Prepost_LagA,df_Prepost_LagB,df_Prepost_LagC)
...
df_PrepostALL$caseTime[df_PrepostALL$case==val_LagA]=result_LagA
df_PrepostALL$caseTime[df_PrepostALL$case==val_LagB]=result_LagB
df_PrepostALL$caseTime[df_PrepostALL$case==val_LagC]=result_LagC
...
for (l in seq(1,nrow(df_Prepost_selected),No_subgroup)){
  ...
  prepost_temp=cbind(datetime=format(as.POSIXct(Group_temp$datetime[1], origin = "1970-01-01"), "%Y-%m-%d"),County=Group_temp$County[1],
                     ...
                     Dementia=sum(Group_temp$Dementia),
                     AS=sum(Group_temp$AS),
                     Schizo=sum(Group_temp$Schizo),
                     Mood=sum(Group_temp$Mood),
                     Stress=sum(Group_temp$Stress),
                     Behavioural=sum(Group_temp$Behavioural),
                     Retardation=sum(Group_temp$Retardation),
                     Others=sum(Group_temp$Others),
...
Prepost_Simplified[,"Dementia"] <- as.numeric(Prepost_Simplified[,"Dementia"])
Prepost_Simplified[,"AS"] <- as.numeric(Prepost_Simplified[,"AS"])
Prepost_Simplified[,"Schizo"] <- as.numeric(Prepost_Simplified[,"Schizo"])
Prepost_Simplified[,"Mood"] <- as.numeric(Prepost_Simplified[,"Mood"])
Prepost_Simplified[,"Stress"] <- as.numeric(Prepost_Simplified[,"Stress"])
Prepost_Simplified[,"Behavioural"] <- as.numeric(Prepost_Simplified[,"Behavioural"])
Prepost_Simplified[,"Retardation"] <- as.numeric(Prepost_Simplified[,"Retardation"])
...
df_PrepostALL[,"Dementia"] <- as.numeric(df_PrepostALL[,"Dementia"])
df_PrepostALL[,"AS"] <- as.numeric(df_PrepostALL[,"AS"])
df_PrepostALL[,"Schizo"] <- as.numeric(df_PrepostALL[,"Schizo"])
df_PrepostALL[,"Mood"] <- as.numeric(df_PrepostALL[,"Mood"])
df_PrepostALL[,"Stress"] <- as.numeric(df_PrepostALL[,"Stress"])
df_PrepostALL[,"Behavioural"] <- as.numeric(df_PrepostALL[,"Behavioural"])
df_PrepostALL[,"Retardation"] <- as.numeric(df_PrepostALL[,"Retardation"])
df_PrepostALL[,"Others"] <- as.numeric(df_PrepostALL[,"Others"])

#df_PrepostALL[,"Insurance_payment"] <- as.numeric(df_PrepostALL[,"Insurance_payment"])   # ------------ Note
...
# Manipulation
df_Geo_Modify=temp_Geo[,c("ID","County","Insurance_payment","Self_payment","zfy")]
df_Geo_Modify$Address=df_Geo$Address
df_Geo_Modify$Latitude=df_Geo$Latitude
df_Geo_Modify$Longitude=df_Geo$Longitude
df_Geo_Modify$NoGeo=df_Geo$NoGeo


# Groupsum by County
disease_sums <- df_Case %>%
  group_by(County) %>%
  summarise(
    Dementia = sum(Dementia, na.rm = TRUE),
    AS = sum(AS, na.rm = TRUE), 
    Schizo = sum(Schizo, na.rm = TRUE),
    Mood = sum(Mood, na.rm = TRUE),
    Stress = sum(Stress, na.rm = TRUE),
    Behavioural = sum(Behavioural, na.rm = TRUE),
    Retardation = sum(Retardation, na.rm = TRUE),
    Others = sum(Others, na.rm = TRUE),
    All_cause = sum(All_cause, na.rm = TRUE)
  )

temp_Geo <- df_Geo_Modify %>%
  left_join(disease_sums, by = "County")

MI_All_cause=Moran.I(temp_Geo$All_cause,Matrix_dists)

MI_Dementia=Moran.I(temp_Geo$Dementia,Matrix_dists)
MI_AS=Moran.I(temp_Geo$AS,Matrix_dists)
MI_Schizo=Moran.I(temp_Geo$Schizo,Matrix_dists)
MI_Mood=Moran.I(temp_Geo$Mood,Matrix_dists)
MI_Stress=Moran.I(temp_Geo$Stress,Matrix_dists)
MI_Behavioural=Moran.I(temp_Geo$Behavioural,Matrix_dists)
MI_Retardation=Moran.I(temp_Geo$Retardation,Matrix_dists)
MI_Others=Moran.I(temp_Geo$Others,Matrix_dists)

MI_result=cbind(MI_Dementia,MI_AS,MI_Schizo,MI_Mood,MI_Stress,MI_Behavioural,
                MI_Retardation,MI_Others,MI_All_cause)
print(MI_result)
...
MI_Lc_Dementia=compute_localMI(temp_Geo$Dementia,temp_Geo$Latitude,temp_Geo$Longitude)
MI_Lc_AS=compute_localMI(temp_Geo$AS,temp_Geo$Latitude,temp_Geo$Longitude)
MI_Lc_Schizo=compute_localMI(temp_Geo$Schizo,temp_Geo$Latitude,temp_Geo$Longitude)
MI_Lc_Mood=compute_localMI(temp_Geo$Mood,temp_Geo$Latitude,temp_Geo$Longitude)
MI_Lc_Stress=compute_localMI(temp_Geo$Stress,temp_Geo$Latitude,temp_Geo$Longitude)
MI_Lc_Behavioural=compute_localMI(temp_Geo$Behavioural,temp_Geo$Latitude,temp_Geo$Longitude)
MI_Lc_Retardation=compute_localMI(temp_Geo$Retardation,temp_Geo$Latitude,temp_Geo$Longitude)
...
MI_Lc_result=cbind(MI_Lc_Dementia,MI_Lc_AS,MI_Lc_Schizo,MI_Lc_Mood,MI_Lc_Stress,MI_Lc_Behavioural,
                   MI_Lc_Retardation,MI_Lc_Others,MI_Lc_All_cause)
print(MI_Lc_result)
...
PSM_Match=function(Prepost,Tier){
  ...
  for(kk in 1:length(unique(Prepost$CountyID))){
    ...
    duplicate_rows <- duplicated(PSM_matchset$County)         # Find the duplicate row
    PSM_matchset <- PSM_matchset[!duplicate_rows, ]    # Keep the unique row
    ...
    m = matchit(Tag~ gdp+GRP_Growth_Rate+Household_Saving+  Population+urban+prate+AgeRatio+EDURatio+work_rate+  GreenRatio+Dim_Env_B+PM25+PM10+SO2+CO+NO2+O3+humidity_Target+precip_Target   +Dim_Hea_A+Health_Cost, 
                data=OrderPSMdata,
                distance='logit',
                method='nearest',
                replace=TRUE,
...
Fill_control=function(GroupT){
  ...
  for(i in 1:nrow(Control_List)){
    ...
    if(length(matching_row)>0){
      ...
      LagA_row=(Start_row+No_subgroup*(Period+post+after)):(Start_row+No_subgroup*(Period+post+after+LagA)-1)
      LagB_row=(Start_row+No_subgroup*(Period+post+after+LagA)):(Start_row+No_subgroup*(Period+post+after+LagA+LagB)-1)
      LagC_row=(Start_row+No_subgroup*(Period+post+after+LagA+LagB)):(Start_row+No_subgroup*(Period+post+after+LagA+LagB+LagC)-1)
      ...
      df_Control_LagA=df_Case[LagA_row,]
      df_Control_LagA$case=val_LagA
      
      df_Control_LagB=df_Case[LagB_row,]
      df_Control_LagB$case=val_LagB
      
      df_Control_LagC=df_Case[LagC_row,]
      df_Control_LagC$case=val_LagC
      
      
      df_Control_temp=rbind(df_Control_Pre,df_Control_Warn,df_Control_Case,df_Control_Post,df_Control_After,df_Control_LagA,df_Control_LagB,df_Control_LagC)
...
Control_select=function(df_ControlALL){
  ...
  df_ControlALL$caseTime[df_ControlALL$case==val_LagA]=result_LagA
  df_ControlALL$caseTime[df_ControlALL$case==val_LagB]=result_LagB
  df_ControlALL$caseTime[df_ControlALL$case==val_LagC]=result_LagC
...
FilePos="~/Library/Mobile Documents/com~apple~CloudDocs/Project/Mental/Result/DID/"
ModelType="/DID_"
ObsWindow='Enduring'
...
#saveRDS(GroupT3,paste(FilePos,Event_type,"/GroupT3.rds",sep = ""))
...
Slots=c('Warning','Post','After','LagA','LagB','LagC') # 'Warning','Post','After','RTH'    
